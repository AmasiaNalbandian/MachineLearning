...
def run_optimization(algorithm_results, algorithm_name):
    ...
    print(f"Problem for {algorithm_name}: {execution_time:.6f} seconds")
    print(f"Execution Time for {algorithm_name}: {execution_time:.6f} seconds")
    print(f"Final Fitness for {algorithm_name}: {best_fitness}")
    print(f"Total Function Evaluations for {algorithm_name}: {len(best_curve)}")
...
def run_rhc(problem, fitness_problem, minimize=False, restarts=[2,5,10,50], max_iters=[50,100, global_max_iters]):
    ...
    for r in restarts:
        ...
        _, rhc_best_curve, rhc_time_log = run_optimization(rhc_results, "RHC")
...
def run_sa(
    ...
    # change the restarts and max iters
    for r in schedule:
        print(f"schedule: {r.__class__.__name__}")
        sa_results = sa(problem=problem, schedule=r)
        _, sa_best_curve, sa_time_log = run_optimization(sa_results, "SA")
    ...
    for iteration in max_attempts:
        print(f"max_attempts: {iteration}")
...
def run_ga(
    ...
    for r in mutation_prob:
        print(f"mutation_prob: {r}")
        ga_results = ga(problem=problem, mutation_prob=r)
        _, ga_best_curve, ga_time_log = run_optimization(ga_results, "GA")
...
def run_ro(
        problem, #the actual reference of fitness prob
        fitness_problem, #name of the problem (i.e. nQueens) 
        alg_results,        #algorithm 
        param_name,         # name of the param we are tuning
        maximize=True,      # is this a max prob
        hyperparameters = [0.2,0.4,0.8], 
        problem_size=150,
        ):
    curves_list = []
    neg_curves_list=[]
    ...
    for param in hyperparameters:
        print(f"problem: {fitness_problem}\t hyperparameter: {param_name}\t = {param}")
        # mimic_results = mimic(problem=problem, keep_pct=r)
        _, best_curve, time_log = run_optimization(alg_results, "mimic")
        curves_list.append({"curve": best_curve, "label": f'mimic keep_pct={r}', "time_log": time_log})
        if not maximize:
            neg_curves_list.append({"curve": -best_curve, "label": f'mimic keep_pct={r}', "time_log": time_log})
    ...
    for iteration in pop_size:
        ...
        _, mimic_best_curve, mimic_time_log = run_optimization(mimic_results, "mimic")
...
mimic_results = mimic(problem=problem, keep_pct=r)
queens_fitness_problem= {'name': "n-Queens", "problem": get_queens_problem()}
