...
def svm(X_train, X_test, y_train, y_test, param_grid, cv=constants.CV):
    ...
    print(f"SVM Boosting took {elapsed_time} seconds to execute.")
    t_start_time = time.time()
    y_train_pred = best_svm_model.predict(X_train)
    t_elapsed_time = time.time() - t_start_time
    print(f"SVM Boosting took {t_elapsed_time} seconds to train.")
...
def get_subset(X_train, y_train, subset_size=1000):
